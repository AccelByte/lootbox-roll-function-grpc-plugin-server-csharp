...
(cd src/AccelByte.PluginArch.LootBox.Demo.Server && dotnet run) & GRPC_SERVER_PID=$!
...
(cd src/AccelByte.PluginArch.LootBox.Demo.Client && dotnet run -- --grpc-target ${NGROK_URL#*://})
